...
public interface CollaborativeDocumentMapper extends BaseMapper<CollaborativeDocument> {
     ...
     * 根据用户ID查询协同文档
     */
    @Select("SELECT * FROM collaborative_documents WHERE owner_id = #{userId} AND is_deleted = 0 ORDER BY update_time DESC")
    List<CollaborativeDocument> selectByCreatorId(@Param("userId") Long userId);
    
    /**
     * 根据文档类型查询
     */
    @Select("SELECT * FROM collaborative_documents WHERE doc_type = #{documentType} AND is_deleted = 0 ORDER BY update_time DESC")
